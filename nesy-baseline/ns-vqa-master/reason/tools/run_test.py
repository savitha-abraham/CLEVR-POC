...
for x, y, ans, idx, complete_image_idx, constraint_type, image in loader:
    ...
    for i in range(pg_np.shape[0]): 
        ...
        pred_ans, flag_reason, flag_error, flag_re, flag_dir, g  = executor.run(pg_np[i], idx_np[i], cs_np[i], ct_p[i],  'val', guess=True)
        
        q_type = find_clevr_question_type(executor.vocab['program_idx_to_token'][y_np[i][1]])
        if ct_p[i] == ct_np[i]:
          stats['correct_cons'] +=1 
        if pred_ans == gt_ans:
            stats['correct_ans']+=1
            if g == 1:
              stats['guess'] += 1 
...
result = {
    ...
    'constraint_acc':stats['correct_cons']/stats['total'],
    'constraint_num':stats['correct_cons'],
